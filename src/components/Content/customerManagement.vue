<template>
  ...
  <div id="customerManagement">
    <div class="Title"><span class="Title_">客户管理</span></div>
    <div class="Table">
      <table class="table_">
        <tr class="search_tr">
      ...
      <div class="dis_data">
        <table class="table">
          <thead>
          <tr class="tr_title">
            <td style="width: 2%" class="td_title"><input type="checkbox" name="category" value="全选" /></td>
          ...
          <tr class="tbody_tr" v-for="row in services">
            <td style="width: 2%"><input type="checkbox" name="category"  value="row['n0']" /></td>
...
export default {
  ...
  methods:{
    getServerData:function(){
      ...
      self.item.search.position=self.$store.state.position;
      console.log(self.item.search.position);
      self.item.pages.Request = self.tfoot.current;
      self.item.pages.each_page = self.tfoot.display;
//          console.log(this.item.pages.each_page);
      let JSONitem = JSON.stringify(self.item,"UTF-8");
